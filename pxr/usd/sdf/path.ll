...
{UTF8U}{UTF8U}*(:{UTF8U}{UTF8U}*)+ {
    ...
    while (current != end)
    {
        if (*current == delim)
        {
            if (!_TfIsValidIdentifier(matchedString, subSequenceBegin, current))
            {
    ...
    return _TfIsValidIdentifier(matchedString, subSequenceBegin, end) ? TOK_NAMESPACED_IDENTIFIER : -1;
}
